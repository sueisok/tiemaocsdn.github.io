...
需要两个字典，两个占位符FUZZ，`product`可以排列组合两个字典，按顺序传入占位符，一共`n*m`条payloads
...
需要两个字典，两个个占位符FUZZ和FU2Z，`zip`可以两个字典一一对应，按顺序传入两个占位符，如果两个字典行数不相等则取行数少的，所以一共`min(n,m)`条payloads
